  ...
  switch (msg->h.event) {
     case CM_EVENT_MSG:
        if (cm.obj[cm_get_handle(cmid)].msg != NULL)
           ...
           xlprint("cm_route() Error : CMID Invalid, CM_EVENT_MSG\n");
        break;
     case CM_EVENT_TIMER:
        if (cm.obj[cm_get_handle(cmid)].timer != NULL)
           cm.obj[cm_get_handle(cmid)].timer(msg);
        else
           xlprint("cm_route() Error : CMID Invalid, CM_EVENT_TIMER\n");
        break;
     default:
...
// 7.26.5   Code
    ...
    delta_us   = (uint32_t)((delta - delta_secs));
